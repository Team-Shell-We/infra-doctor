...
// 회귀 테스트: 아직 도입 안 한 topic(예: k8s가 없는 프로젝트에서 explain k8s)의
// 경우, AI가 build_flow/why_topic을 이미 일어나는 일처럼 서술하면
// "현재 상태"의 ✗와 모순돼 보인다. absent로 표시된 사실은 가정법으로
// 서술하라는 지시가 시스템 프롬프트에 있어야 한다.
func TestSystemPromptInstructsHypotheticalPhrasingForAbsentFacts(t *testing.T) {

	req, err := BuildRequest("k8s", ai.Summary{}, []StatusItem{{Label: "Kubernetes manifests", Present: false}}, "en")
	if err != nil {
		t.Fatalf("BuildRequest failed: %v", err)
	}

	if !strings.Contains(req.SystemPrompt, "absent") {
		t.Error("SystemPrompt should instruct the model to treat facts marked absent differently from present ones")
	}

	if !strings.Contains(strings.ToLower(req.SystemPrompt), "would") {
		t.Error("SystemPrompt should tell the model to use hypothetical phrasing (e.g. \"would\") for absent facts")
	}
}
